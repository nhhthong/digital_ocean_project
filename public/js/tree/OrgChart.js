...
OrgChart.prototype.load = function(t) {
    ...
    this.server.read(t, this.width(), this.height(), e, O, v, function(t) {
        O != OrgChart.action.exporting && (y.nodes = t.nodes), y.editUI.fields = t.allFields;
        var e = {
            defs: ""
        };
        ...
        for (var n in OrgChart.events.publish("prerender", [y, e]), r = e.content, t.visibleNodes) {
            var l = t.visibleNodes[n],
                s = y._get(n);
            r += y.ui.node(l, s, t.animations, y.config, void 0, void 0, void 0, O, i, y), r += y.ui.link(l, y, y.config, i), r += y.ui.expandCollapse(l, y.config, O, i)
        }
        e = {
            content: r,
            res: t
        };
        if (OrgChart.events.publish("render", [y, e]), r = e.content, r += y.ui.lonely(y.config), O !== OrgChart.action.exporting) {
            O !== OrgChart.action.centerNode && O !== OrgChart.action.insert && O !== OrgChart.action.expandCollapse && O !== OrgChart.action.update || (o = a), O === OrgChart.action.init && null != a && (o = a), y.response = t;
            m = y.ui.svg(y.width(), y.height(), o, y.config, r);
            if (y._initialized) {
                var h = y.getSvg(),
                    d = h.parentNode;
                d.removeChild(h), d.insertAdjacentHTML("afterbegin", m), y._attachEventHandlers(), y.xScrollUI.addListener(y.getSvg()), y.yScrollUI.addListener(y.getSvg()), y.xScrollUI.setPosition(), y.yScrollUI.setPosition()
            } else y.element.innerHTML = y.ui.css() + y.ui.exportMenuButton(y.config) + m, y._attachInitEventHandlers(), y._attachEventHandlers(), y.xScrollUI.create(y.width(), y.config.padding), y.xScrollUI.setPosition(), y.xScrollUI.addListener(y.getSvg()), y.yScrollUI.create(y.height(), y.config.padding), y.yScrollUI.setPosition(), y.yScrollUI.addListener(y.getSvg()), y.config.enableSearch && y.searchUI.addSearchControl(), y.toolbarUI.init(y, y.config.toolbar);
            var c = !1;
            y.notifierUI.show(t.notif);
            var g = y.response.animations;
            if (0 < g[0].length) {
                y._hideBeforeAnimation(g[0].length);
                for (var p = 0; p < g[0].length; p++) g[0][p] = y.getNodeElement(g[0][p]);
                OrgChart.anim(g[0], g[1], g[2], y.config.anim.duration, y.config.anim.func, function() {
                    c || (x && x(), OrgChart.events.publish("redraw", [y]), y._showAfterAnimation(), c = !0)
                })
            }
            O === OrgChart.action.centerNode ? OrgChart.anim(y.getSvg(), {
                ...
                y.ripple(v.options.rippleId), c || (x && x(), OrgChart.events.publish("redraw", [y]), y._showAfterAnimation(), c = !0)
            }, function() {
                y.xScrollUI.setPosition(), y.yScrollUI.setPosition()
            }) : !a || !y.response || a[0] == y.response.viewBox[0] && a[1] == y.response.viewBox[1] && a[2] == y.response.viewBox[2] && a[3] == y.response.viewBox[3] || O !== OrgChart.action.insert && O !== OrgChart.action.expandCollapse && O !== OrgChart.action.update && O !== OrgChart.action.init ? 0 == g[0].length && (c || (x && x(), OrgChart.events.publish("redraw", [y]), c = !0)) : OrgChart.anim(y.getSvg(), {
                viewbox: a
            }, {
                viewbox: y.response.viewBox
            }, 500, OrgChart.anim.inOutPow, function() {
                y.xScrollUI.setPosition(), y.yScrollUI.setPosition(), c || (x && x(), OrgChart.events.publish("redraw", [y]), c = !0)
            }), y._initialized || (y._initialized = !0, OrgChart.events.publish("init", [y]))
        } else {
            var f = t.boundary,
                u = f.maxX - f.minX,
                C = f.maxY - f.minY,
    ...
    this.element.style.position = "relative", 0 == this.element.offsetHeight && (this.element.style.height = "100%", 0 == this.element.offsetHeight && (this.element.style.height = "1200px")), 0 == this.element.offsetWidth && (this.element.style.width = "100%", 0 == this.element.offsetWidth )
            ...
            if (y) {
                if (y.parent)
                    for (var b = (H = y.parent).children.length - 1; 0 <= b; b--) {
                        (z = H.children[b]) == y && (z.parent = null, H.children.splice(b, 1), H.childrenIds.splice(b, 1))
                    }
                y.isAssistant = !1, l.push(y)
            }
        }
    }
    ...
    if ("multiline" == s) {
        ...
        for (var O = 1, v = 1; v < f.length; v++) {
            ...
            if (C.firstChild.data += " " + f[v], C.getComputedTextLength() > l) {
                if (C.firstChild.data = C.firstChild.data.slice(0, x), O++, 0 !== c && c < O) break;
    ...
    function t(t, e, r) {
        ...
        var x = i.join(" ");
        if (A += '<g c-link-from="{from}" c-link-to="{to}">'.replace("{from}", t.id).replace("{to}", e.id) + n.link.replace("{d}", x), N.label) {
